    ...
    try:
        if request.method == "POST":
            ...
            if len(username) >2 and len(password) >4:
                user.Login = request.POST.get("log")
                ...
                messages.error(request, 'Login and Password must be entered correctly')
        ...
        messages.error(request, 'User with this Login and Password has already signed up')
        return render(request, "login.html", data)
...
def Calculate(request):
    if request.method == "POST":
        ...
        notCareSeq = request.POST.get("notCareSeq")
        try:
            checkUserLogin = User.objects.get(Login=username, Password=password)
            if checkUserLogin is not None:
                string = str(sequence)
                string1 = str(notCareSeq)
                ans = getResult(string, string1)
                data = {
                    'log': username,
                    'pass': password,
                    'sequence': sequence,
                    'notCareSeq': notCareSeq,
                    'answer': ans,
                }
                if ans=='':
                ...
                try:
                    checkSequence = requests.objects.filter(sequence=sequence, result=ans)
                    if checkSequence is not None:
                        data = {
                            'log': username,
                            'pass': password,
                            'sequence': sequence,
                        ...
                        messages.error(request, 'Answer will be appear from database!')
                        return render(request, "main.html", data)
                    ...
                    tmp = requests.objects.create(sequence=sequence, result=ans,
                                                   Username_Id=checkUserLogin.id)
                    messages.error(request, 'Data collected succesfully!')
                    return render(request, "main.html", data)
...
def showRequests(request):
    if request.method == "POST":
        ...
        try:
            checkUserLogin = User.objects.get(Login=username, Password=password)
            if checkUserLogin is not None:
                data = requests.objects.filter(Username_Id=checkUserLogin.id)
            ...
            messages.error(request, "Incorrect Login or Password")
            return render(request, "requests.html", data)
    return HttpResponseRedirect("http://127.0.0.1:8000/requests")
...
def mainFunc(string, string1):
    if string=='':
        ans = ''
        return ans
    mt = [int(i) for i in string.strip().split()]
    dc = [int(i) for i in string1.strip().split()]
    mt.sort()
    minterms = mt + dc
    minterms.sort()
    ...
    ans = 'F = ' + ' + '.join(''.join(i) for i in final_result)
    return ans

def getResult(string, string1):
    res = mainFunc(string, string1)
    return res